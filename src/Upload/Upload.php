...
class Upload
{
     ...
     */
    public function resizeRatio (int $W_max, int $H_max, string $rep_Dst, string $img_Dst, string $rep_Src, string $img_Src)
    {
        $condition = 0;
...
// 	---------------------------------------------------------------
     ...
     */
    public function resizeCenter (int $W_fin, int $H_fin, string $rep_Dst, string $img_Dst, string $rep_Src, string $img_Src)
    {
        // ---------------------
        $condition = 0;
     ...
     * @return bool
     */
    function picto (string $chaine, string $rep_Dst, string $img_Dst, string $rep_Src, string $img_Src, string $position = '', bool $image = false)
    {
        $condition = 0;
     ...
     * @return string
     */
    public function getUploadExtension (string $type)
    {
     ...
     */
    public function createUploadFolder (string $dir_name)
    {
     ...
     * @return null
     */
    public function savePicture (string $file, int $id, array $options = [], array $erreurs = [])
    {
        ...
        if (isset($_FILES[$file]) && $_FILES[$file]['error'] != UPLOAD_ERR_NO_FILE) {
            ...
            //Recuperation de la dimension de l'image actuelle
            if ($resize) {
            ...
            if ($_FILES[$file]['size'] <= $this->upload_max_size) {
                ...
                if (in_array($extension_upload, $this->upload_extension($autorist_file_type))) {
                    ...
                    if (!isset($resize_w_size, $resize_h_size) || (($width > $resize_w_size) && ($height > $resize_h_size))) {
                        ...
                        //Creation du dossier de l'image à partir de l'id de l'annonce
                        if ($directory) {
                                ...
                                elseif ($resize) {
                                    ...
                                    if ($resize && ucfirst($resize_type) === 'Center') {
                                        ...
                                        if($width < $resize_w_size && $height >= $resize_h_size){
                                            $this->resizeRatio($resize_w_size, 0, $resize_directory, '', $upload_directory, $newName);
                                        }
                                        elseif($height < $resize_h_size && $width >= $resize_w_size){
                                            $this->resizeRatio(0, $resize_h_size, $resize_directory, '', $upload_directory, $newName);
                                        }
                                        elseif($height < $resize_h_size && $width < $resize_w_size){
                                            if($height < $width){
                                                $this->resizeRatio(0, $resize_h_size, $resize_directory, '', $upload_directory, $newName);
     ...
     * @return array
     */
    protected function upload_extension (string $type)
    {
        if ($type === 'picture') {
            ...
            $extension = ['jpg', 'jpeg', 'gif', 'png', 'txt', 'doc', 'ppt', 'xls', 'pdf', 'odt', 'docx',
                'xlsx', 'pptx', 'psd'];
        } else {
            $extension = ['jpg', 'jpeg', 'gif', 'png', 'txt', 'doc', 'ppt', 'xls', 'pdf', 'odt', 'docx',
                'xlsx', 'pptx', 'psd'];
